...
//		float esieper=4.75f;
		...
		float reduction[] = new float[5000];
		...
		String sf[] = new String[5000];
		int mondays=31;
		int enggexp[]=new int[5000];
		int enggexpmonth[]=new int[5000];
		String vmonth="";
		String vvmonth="";
		String error="";
		String error1="";
		String vacademicyear=""; 
		String vvacademicyear=""; 
		String pfname[] = new String[5000];
		String pfparentname[] = new String[5000];
		String pfparentorhusband[] = new String[5000];
		String pfaccno[] = new String[5000];
		int pfwages[]=new int[5000];
		int pfepf[]=new int[5000];
		int pfdiff[]=new int[5000];
		int pfpension[]=new int[5000];
		int pflop[]=new int[5000];
		String pfmon[]=new String[5000];
		String pfacademicyear[]=new String[5000];
...
String dnoin="0";
   ...
   float ff(float num)
     ...
     {
	    	vvmonth="APR";
	    	vvacademicyear=vacademicyear.substring(8,12)+" -- "+(Integer.parseInt(vacademicyear.substring(8,12))+1);
     }
		...
		String ssalary="";
		if(dnoin.equals("0,All"))
			
		ssalary="select staff.staffid,consoldate,dayslossofpay,accountno,bank,basic,ma,others,pp,"+
		...
		"staffdesignation.cname,department.dname,enggexp+otherexp,enggexpmonth+otherexpmonth,scsa,splallow,sf from staffdesignation,department,salary,"+
		"staff,salarycategory where salary.staffid=staff.staffid and "+
		"staff.designation=staffdesignation.slno and staff.dno=department.dno "+
		"and salary.scale=salarycategory.slno and salary.dayslossofpay<> "+mondays+" and salary.month='"+month+
		"' and salary.academicyear='"+academicyear+"' order by dname,staffdesignation.slno,staff.staffid";
else
		ssalary="select staff.staffid,consoldate,dayslossofpay,accountno,bank,basic,ma,others,pp,"+
		"rd,lic,loan,pfper,fa,society,club,spclub,saladvance,telephone,cooptex,khadi,"+
		"protax,incometax,processfee,others1,salarycategory.cname,name,basicqual,desiredqual,"+
		...
		"staffdesignation.cname,department.dname,enggexp+otherexp,enggexpmonth+otherexpmonth,scsa,splallow,sf from staffdesignation,department,salary,"+
		"staff,salarycategory where salary.staffid=staff.staffid and "+
		"staff.designation=staffdesignation.slno and staff.dno=department.dno and staff.dno in ("+dnoin+")"+
		"and salary.scale=salarycategory.slno and salary.dayslossofpay<> "+mondays+" and salary.month='"+month+
		"' and salary.academicyear='"+academicyear+"' order by dname,staffdesignation.slno,staff.staffid";
		    	...
		    	try
		    	{
					...
					while(rs.next())
						...
						dayslossofpay[count]=rs.getFloat(3);
						...
						enggexp[count]=rs.getInt(39);
						enggexpmonth[count]=rs.getInt(40);
						scsaper[count]=rs.getInt(41);
						splallow[count]=rs.getInt(42);
						sf[count]=rs.getString(43);
						count++;	
						}
					rs.close();
					stmt.close();
...
//						if(staffid[i].charAt(1)=='N')percentage5[i]=(float)(gtot[i]*2/100);
				...
				reduction[i]=(lic[i] + pf[i] + esi[i] + fa[i] + percentage5[i] + society[i] + club[i] + spclub[i] + saladvance[i] + telephone[i] + cooptex[i] + khadi[i] + protax[i] + incometax[i] + processfee[i]+others1[i]+rd[i]+loan[i]);
				netpay[i]=(gtot[i] - reduction[i]);
error1="first";
if(pf[i]==780) 
{
	pfwages[i]=6500;
...
if(pf[i]>0) 
{
    pfwages[i]=Math.round((basic[i]+da[i])/mondays*(mondays-dayslossofpay[i]));
    //pfwages[i]=basic[i]+da[i];
	pfepf[i]=Math.round(pf[i]);
	pfpension[i]=(int)Math.round((((basic[i]+da[i])/mondays*(mondays-dayslossofpay[i]))*.0833));
	pfdiff[i]=Math.round(pf[i])-pfpension[i];
}
}
	pflop[i]=(int)dayslossofpay[i];
error1="second";

	ps =con.prepareStatement("delete from pf where staffid=? and pfmon=? and academicyear=?");
	ps.setString(1,staffid[i]);
	ps.setString(2,vmonth);
	ps.setString(3,vacademicyear);
	ps.execute();
	ps.close();
error1="one";
	ps =con.prepareStatement("insert into pf values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	ps.setString(1,staffid[i]);
	ps.setInt(2,pfwages[i]);
	ps.setInt(3,pfepf[i]);
	ps.setInt(4,pfdiff[i]);
	ps.setInt(5,pfpension[i]);
	ps.setInt(6,pflop[i]);
	ps.setString(7,vmonth);
	ps.setString(8,vacademicyear);
	ps.setFloat(9,basic[i]);
	ps.setFloat(10,da[i]);
	ps.setFloat(11,hra[i]);
	ps.setFloat(12,pf[i]);
	ps.setFloat(13,lic[i]);
	ps.setFloat(14,tds[i]);
...
con.close();
}catch(Exception e){error=error1+e.toString();}
finally{}
		...
		public String AllView(String staff,String academicyear)
		    {
String ssalary="";
		if(dnoin.equals("0,All"))
		ssalary="select staff.staffid,consoldate,dayslossofpay,accountno,bank,basic,ma,others,pp,"+
		...
		"protax,incometax,processfee,others1,salarycategory.cname,name,basicqual,desiredqual,"+
		"researchqual,addqual,desiredquala,researchquala,addquala,birthdate,appdate,"+
		"staffdesignation.cname,department.dname,enggexp+otherexp,enggexpmonth+otherexpmonth,month,sf from staffdesignation,department,salary,"+
		"staff,salarycategory where salary.staffid=staff.staffid and "+
		"staff.designation=staffdesignation.slno and staff.dno=department.dno "+
		"and salary.scale=salarycategory.slno and staff.staffid='"+staff+
		"' and salary.academicyear='"+academicyear+"' order by dname,staffdesignation.slno,staff.staffid";
else
		ssalary="select staff.staffid,consoldate,dayslossofpay,accountno,bank,basic,ma,others,pp,"+
		"rd,lic,loan,pfper,fa,society,club,spclub,saladvance,telephone,cooptex,khadi,"+
		...
		"' and salary.academicyear='"+academicyear+"' order by dname,staffdesignation.slno,staff.staffid";
		    	try
		    	{
					...
					while(rs.next())
						...
						enggexp[count]=rs.getInt(39);
						enggexpmonth[count]=rs.getInt(40);
						pfmon[count]=rs.getString(41);
						sf[count]=rs.getString(42);
						count++;	
						}
					rs.close();
					stmt.close();
...
public String pfView(String academicyear)
		    {
        String pacademic=Integer.parseInt(academicyear.substring(0,4))-1+" -- "+ Integer.parseInt(academicyear.substring(0,4));
		String ssalary="select max(pf.staffid),max(staff.name),max(staff.parentname),max(staff.parentorhusband),max(bank),max(pfwages),max(pfepf),max(pfdiff),max(pfpension)," +
                      " max(pflop),max(pfmon),pf.academicyear,max(pf.basic),max(pf.da),max(pf.hra),max(pf.pf),max(pf.lic),max(pf.tds),max(others),(max(scsa)*(max(pf.basic)+max(others)))/100,max(ma),max(splallow) " + 
                      " from pf,salary,staff where staff.staffid=pf.staffid and salary.staffid=pf.staffid " +
//                      " and pf.academicyear='"+academicyear+"' group by pf.staffid,pf.academicyear,pf.pfmon order by pf.staffid";
                      "  and ((pf.academicyear='"+academicyear+"' and  pfmon<>'MAR') or (pf.academicyear='"+pacademic+"' and pfmon='MAR')) group by pf.staffid,pf.academicyear,pf.pfmon order by pf.staffid";
		    	...
		    	try
		    	{
					...
					while(rs.next())
						{
						staffid[count]=rs.getString(1);
		                pfname[count] = rs.getString(2);
						pfparentname[count] =rs.getString(3);
						pfparentorhusband[count] =rs.getString(4);
						pfaccno[count] =rs.getString(5);
						pfwages[count]=rs.getInt(6);
						pfepf[count]=rs.getInt(7);
						pfdiff[count]=rs.getInt(8);
						pfpension[count]=rs.getInt(9);
						pflop[count]=rs.getInt(10);
						pfmon[count]=rs.getString(11);
						pfacademicyear[count]=rs.getString(12);
						basic[count]=rs.getFloat(13);
						da[count]=rs.getFloat(14);
						hra[count]=rs.getFloat(15);
						pf[count]=rs.getFloat(16);
						lic[count]=rs.getFloat(17);
						tds[count]=rs.getFloat(18);
						others[count]=rs.getFloat(19);
						scsa[count]=rs.getFloat(20);
						ma[count]=rs.getFloat(21);
						splallow[count]=rs.getInt(22);
						count++;	
						}
					rs.close();
					stmt.close();
	...
	public int[] getenggexp()
	{
		return this.enggexp;
	}
	public int[] getenggexpmonth()
	{
		return this.enggexpmonth;
	}
	...
	public String[] getpfparentname()
	{
		return this.pfparentname;
	}
	public String[] getpfparentorhusband()
	{
		return this.pfparentorhusband;
	}
	public String[] getpfaccno()
	{
		return this.pfaccno;
	}
	public int[] getpfwages()
	{
		return this.pfwages;
	}
	public int[] getpfepf()
	...
	public int[] getpfdiff()
	{
		return this.pfdiff;
	}
	public int[] getpfpension()
	{
		return this.pfpension;
	}
	public int[] getpflop()
	{
		return this.pflop;
	}
	public String[] getpfmon()
	{
		return this.pfmon;
	}
	public String[] getpfacademicyear()
	{
		return this.pfacademicyear;
	}

...
public class FineClassViewAll
	...
	int year=0;
				int yyear=0;
	String academicyear="";
	int rollno[] = new int[3000];
	String name[]= new String[3000];
	int cyear[] = new int[3000];
	float fine[][] = new float[3000][50];
	float totfine[] = new float[3000];
	int slno[] = new int[50];
	String quer="";
	String sex[] = new String[3000];
	String fname[] = new String[50];
	float paid[] = new float[3000];
	int library[] = new int[3000];
	int hod[] = new int[3000];
	float balance[] = new float[3000];
	int accounts[] = new int[3000];
	int id=0;
		...
		public int[] getRollnoAll()
			{
				...
				try
					...
					ResultSet rs = stmt.executeQuery("select stud.rollno,stud.name,sex,year from stud,student where stud.rollno=student.rollno and active=1 and academicyear in(select academicyear from academicyear) order by stud.rollno");
					while(rs.next())
						{
						rollno[count]=rs.getInt(1);
						name[count]=rs.getString(2);
						sex[count]=rs.getString(3);
						cyear[count]=rs.getInt(4);
						count++;	
						}
					rs.close();
					stmt.close();
...
if(cyear[j]==1) quer="select sum(amount) from accounts.dbo.feesstructureamount where fsino="+feesslno+" and fshno not in(140,155,156,157,159,160,161,1010,1002,1006,1011,1003,1007,1012,1004,1008,1014,1015,1016,117,151,120,147,123,126,129,118,152,121,148,124,127,131,119,153,122,149,125,128,130)";

if(cyear[j]==2) quer="select sum(amount) from accounts.dbo.feesstructureamount where fsino="+feesslno+" and fshno not in(140,156,157,160,161,1011,1003,1007,1012,1004,1008,1015,1016,118,152,121,148,124,127,131,119,153,122,149,125,128,130)";

if(cyear[j]==3) quer="select sum(amount) from accounts.dbo.feesstructureamount where fsino="+feesslno+"  and fshno not in(140,157,161,1012,1004,1008,1016,119,153,122,149,125,128,130)";

if(cyear[j]==4) quer="select sum(amount) from accounts.dbo.feesstructureamount where fsino="+feesslno;
...
if(cyear[j]==1) quer="SELECT     SUM(amount) FROM accounts.dbo.feesstructureindividualamount,accounts.dbo.feesstructureindividualidentify WHERE   fsiino=slno and rollno="+rollno[j]+" and fshno not in(140,155,156,157,159,160,161,1010,1002,1006,1011,1003,1007,1012,1004,1008,1014,1015,1016,117,151,120,147,123,126,129,118,152,121,148,124,127,131,119,153,122,149,125,128,130)";
...
if(cyear[j]==2) quer="SELECT     SUM(amount) FROM accounts.dbo.feesstructureindividualamount,accounts.dbo.feesstructureindividualidentify WHERE   fsiino=slno and rollno="+rollno[j]+" and fshno not in(140,156,157,160,161,1011,1003,1007,1012,1004,1008,1015,1016,118,152,121,148,124,127,131,119,153,122,149,125,128,130)";
...
if(cyear[j]==3) quer="SELECT     SUM(amount) FROM accounts.dbo.feesstructureindividualamount,accounts.dbo.feesstructureindividualidentify WHERE   fsiino=slno and rollno="+rollno[j]+" and fshno not in(140,157,161,1012,1004,1008,1016,119,153,122,149,125,128,130)";
...
if(cyear[j]==4) quer="SELECT     SUM(amount) FROM accounts.dbo.feesstructureindividualamount,accounts.dbo.feesstructureindividualidentify WHERE   fsiino=slno and rollno="+rollno[j];
		...
		public void FeeUpdate()
			{
				...
				try
					...
					{
					try{
	                    sql="insert into feesbalance values(?,?)";
						ps=con.prepareStatement(sql);
						ps.setString(1,rollno[i]+"");
						ps.setString(2,(totfine[i]-paid[i])+"");
						int j = ps.executeUpdate();
						ps.close();
					}catch(Exception e){e.toString();}					
					try{
	                    sql="update feesbalance set balance=? where rollno=?";
						ps=con.prepareStatement(sql);
						ps.setString(1,(totfine[i]-paid[i])+"");
						ps.setString(2,rollno[i]+"");
						int j = ps.executeUpdate();
						ps.close();
					}catch(Exception e){e.toString();}					

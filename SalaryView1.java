...
String dnoin="0";
   ...
   float ff(float num)
      ...
      if(num-(float)(int)(num)>.49)
		...
		"and salary.scale=salarycategory.slno and salary.dayslossofpay<> "+mondays+" and salary.month='"+month+"' and salary.academicyear='"+academicyear+"' order by staff.staffid";
else
		ssalary="select staff.staffid,consoldate,dayslossofpay,accountno,bank,basic,ma,others,pp,"+
		"rd,lic,loan,pfper,fa,society,club,spclub,saladvance,telephone,cooptex,khadi,"+
		"protax,incometax,processfee,others1,salarycategory.cname,name,basicqual,desiredqual,"+
...
//				pf[i]=(basic[i]+da[i])/mondays*(mondays-dayslossofpay[i])*pfper[i]/100;if(pf[i]>780)pf[i]=780;
				pf[i]=ff(pf[i]);
				esi[i]=ff(esi[i]);
				esie[i]=ff(esie[i]);
				pf[i]+=rd[i];rd[i]=0;
